...
def wbw_dsplint(xa,ya,y2a,nx,x):
   ...
   da = -1.0/h
   db =  1.0/h
   dy=da*ya[klo]+db*ya[khi]+((da*3*a**2-da)*y2a[klo]+(db*3*b**2-db)*y2a[khi])*h**2/6.0
   return dy
   ...
   for jj in range(nion-1):
      for ii in range(jj+1,nion):
         ...
         if (dij < cutoff2):
            ...
            nlist[jj] += 1
            if (nn[ii]==-1): nn[ii] = [jj]
            else:            nn[ii].append(jj)
            if (nn[jj]==-1): nn[jj] = [ii]
            else:            nn[jj].append(ii)
      ...
      if (nlist[ii]>1):
         for j in range(nlist[ii]-1):
            ...
            for k in range(j+1,nlist[ii]):
               ...
               f[3*ii]   += -tmp4*((dxij+dxik)/(dij*dik) - tmp5*(dxij/(dij*dij)+dxik/(dik*dik)))
               f[3*ii+1] += -tmp4*((dyij+dyik)/(dij*dik) - tmp5*(dyij/(dij*dij)+dyik/(dik*dik)))
               f[3*ii+2] += -tmp4*((dzij+dzik)/(dij*dik) - tmp5*(dzij/(dij*dij)+dzik/(dik*dik)))
...
def make_nwinput(PORT,job):
   ss = ""
   if (job['theory']=='pspw'):   ss += "title \"automatic pspw nwchemjob\"\n\n" + "start nwchempspw%d\n\n"  % PORT
   elif (job['theory']=='dft'):  ss += "title \"automatic dft  nwchemjob\"\n\n" + "start nwchemdft%d\n\n"   % PORT
   elif (job['theory']=='mp2'):  ss += "title \"automatic mp2  nwchemjob\"\n\n" + "start nwchemmp2%d\n\n"   % PORT
   elif (job['theory']=='scf'):  ss += "title \"automatic scf  nwchemjob\"\n\n" + "start nwchemscf%d\n\n"   % PORT
   else:                         ss += "title \"automatic nwchemjob\"\n\n"      + "start nwchemother%d\n\n" % PORT
   ss += "memory 1500 mb\n\n" 
   ...
   if (job['theory']=='dft'):
      if (job.has_key('basis')): ss += job['basis'] + "\n\n"
      else: ss += "basis \"ao basis\" cartesian print\n" + "  * library \"6-311++G(2d,2p)\"\n" + "end\n\n" 
      ss += "dft\n"
   ...
   if ((njobs==1) and (not onlyroute)):
      ...
      if (ok):
         ...
         for s in ss:
            ...
            egrads += data
            if (data==[None]): badjobs.append(jobs[istart:iend])
            else:              newhostsports.append(hostsports[j])
            istart = iend 
            i += 1
            j += 1
            s.close()
...
if (len(sys.argv)>=2):
   ...
   if (runtype.find('stop') !=-1):
      ...
      if onlyroute: print '              - port ',PORT,' routing to ',hostsports
      else:         print '              - port ',PORT,' using ',hostsports
